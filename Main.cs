        ...
        private void SearchSubtitleThread()
        {
            try
            {
                ...
                Invoke((EventHandler)delegate
                {
                    ...
                    fs.Subtitle_Button_AutoGet.Enabled = false;
                ...
                Invoke((EventHandler)delegate
                {
                    fs.ComboBox_Select_Subtitles.BeginUpdate();
                    fs.ComboBox_Select_Subtitles.Items.Clear();
                    foreach (var sb in res.sublist)
                    {
                        if (string.IsNullOrEmpty(sb.sname) || string.IsNullOrEmpty(sb.surl))
                            continue;
                        ComboBoxItem cbi = new ComboBoxItem();
                        cbi.Text = "[" + sb.language + "]" + sb.sname;
                        cbi.Value = sb.surl;
                        fs.ComboBox_Select_Subtitles.Items.Add(cbi);
                        //fs.ToolTip.SetToolTip(cbi, cbi.Text);
                    }
                    ShowPictureText("已找到匹配字幕 - " + fs.ComboBox_Select_Subtitles.Items.Count + "个");
                    fs.ComboBox_Select_Subtitles.EndUpdate();
            ...
            finally
            {
                ...
                {
                    fs.Subtitle_Main_IsProcess.Visible = false;
                    fs.Subtitle_Button_AutoGet.Enabled = true;
                });
            }
        }
