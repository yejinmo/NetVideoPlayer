...
namespace NetVideoPlayer
{
    public class Player : AxPlayer
    {
        ...
        public Player()
        {
            _TimerDownloadSpeed = new System.Threading.Timer(_DownloadSpeed, this, 1000, 1000);
        }
        ...
        /// <returns></returns>
        private static string _ConvertTime(int time)
        {
            bool ShowMsec = true; //是否显示毫秒
        ...
        {
            return base.GetVideoWidth().ToString() + 'x' + base.GetVideoWidth().ToString();
        }
        ...
        /// 显示文字定时器
        /// </summary>
        private System.Threading.Timer _TimerPictureTextClose;

        /// <summary>
        /// 提示文字队列
        /// </summary>
        private Queue<string> _QueuePcitureText = new Queue<string>();
        ...
        public void _ShowPictureText(string text, int time = 2000)
        {
            _QueuePcitureText.Enqueue(text + '\n');
            string str = "";
            foreach (string i in _QueuePcitureText)
                str = str + i;
            SetPictureEnable(true);
            SetPictureText(str);
            if (_TimerPictureTextClose == null)
                _TimerPictureTextClose = new System.Threading.Timer(new TimerCallback(_PictureTextClose), this, 0, time);
        }
        ...
        private void _PictureTextClose(object obj)
        {
            string str = "";
            foreach (string i in _QueuePcitureText)
                str = str + i;
            SetPictureEnable(true);
            SetPictureText(str);
            if (_QueuePcitureText.Count != 0)
                _QueuePcitureText.Dequeue();
        }

        /// <summary>
        /// 立即刷新提示文字
        /// </summary>
        public void _ShowPictureTextNow()
        {
            string str = "";
            foreach (string i in _QueuePcitureText)
                str = str + i;
        ...
        private System.Threading.Timer _TimerDownloadSpeed;

        private long _LastDownloadSpeed = 0;

        private double _CurrentDownloadSpeed = 0;

        private void _DownloadSpeed(object obj)
        {
            if (GetState() != EnumPlayState.IsPlay)
                return;
            long temp = GetReadSize();
            _CurrentDownloadSpeed = (temp - _LastDownloadSpeed) / 1024;
            _LastDownloadSpeed = temp;
        }
        ...
        /// 单位 KB/S
        /// </summary>
        /// <returns>当前下载速度</returns>
        public double _GetCurrentDownloadSpeed()
        {
            return Math.Abs(_CurrentDownloadSpeed);
        }
        ...
        public int _VolumeUp(int step = 1, bool AutoTip = true, string TipHead = "音量：", int time = 2000)
        {
            int CurrentVolumeUp = GetVolume();
            SetVolume(CurrentVolumeUp += step);
            if (AutoTip)
            {
                if (_QueuePcitureText.Count > 0 && _QueuePcitureText.ToArray()[_QueuePcitureText.Count - 1].Contains(TipHead))
                {
                    _QueuePcitureText.ToArray()[_QueuePcitureText.Count - 1] = TipHead + CurrentVolumeUp.ToString() + '\n';
                    _ShowPictureTextNow();
                }
                else
                    _ShowPictureText(TipHead + CurrentVolumeUp.ToString(), time);
            }
        ...
        /// <param name="time">提示文字持续时间，仅当AutoTip为真时生效</param>
        /// <returns>执行操作后的音量值</returns>
        public int _VolumeDown(int step = 1, bool AutoTip = true, string TipHead = "音量：", int time = 2000)
        {
            int CurrentVolumeUp = GetVolume();
            SetVolume(CurrentVolumeUp -= step);
            if (AutoTip)
            {
                if (_QueuePcitureText.Count > 1 && _QueuePcitureText.ToArray()[_QueuePcitureText.Count - 1].Contains(TipHead))
                    _QueuePcitureText.ToArray()[_QueuePcitureText.Count - 1] = TipHead + CurrentVolumeUp.ToString() + '\n';
                else
                    _ShowPictureText(TipHead + CurrentVolumeUp.ToString(), time);
            }
        ...
        private bool _MatchHash(string url)
        {
            string hash = string.Empty;
            Regex hashrg = new Regex("[0-9a-z]{40}");
            Match mc = hashrg.Match(url.ToLower());
            if (mc.Length > 0)
            {
                string s = mc.Groups[0].Value;
                hash = s;
                return true;
            }
            else
        ...
        private string _GetThunderHash(string url)
        {
            WebClient web = new WebClient();
            string xunlei = web.DownloadString(string.Format("http://i.vod.xunlei.com/req_subBT/info_hash/{0}/req_num/1000/req_offset/0", hash));
            if (string.IsNullOrEmpty(xunlei) || xunlei.Contains("\"record_num\": 0"))
            {
                return false;
            ...
            XunleiItem xl = JsonHelper.ParseFormByJson<XunleiItem>(xunlei);
            return true;
        }
